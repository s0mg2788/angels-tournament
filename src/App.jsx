...
function ConfirmModal({message,onConfirm,onCancel}){
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:"16px"}}>
      <div style={{background:"#1a1a2a",border:"1px solid rgba(239,68,68,.35)",borderRadius:"12px",padding:"24px",maxWidth:"300px",width:"100%"}}>
        <div style={{fontSize:"22px",marginBottom:"10px",textAlign:"center"}}>⚠️</div>
        <div style={{fontSize:"13px",color:"rgba(255,255,255,.7)",lineHeight:1.6,textAlign:"center",marginBottom:"20px",whiteSpace:"pre-line"}}>{message}</div>
        <div style={{display:"flex",gap:"10px"}}>
          <button onClick={onCancel} style={{flex:1,padding:"10px",borderRadius:"7px",border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.07)",color:"#e8e8f0",cursor:"pointer",fontWeight:600,fontSize:"13px"}}>Hủy</button>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    style.textContent=`*{box-sizing:border-box;margin:0;padding:0}select option{background:#1a1a2a;color:#e8e8f0}input[type=number]::-webkit-inner-spin-button{opacity:.4}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:rgba(212,175,55,.3);border-radius:2px}@keyframes fadeIn{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:none}}.fade{animation:fadeIn .25s ease}@keyframes spin{to{transform:rotate(360deg)}}`;
  ...
  // ── Design tokens ──────────────────────────────────────────
  const G="#D4AF37",GR="#4ade80",RE="#f87171",TX="#e8e8f0",DM="rgba(255,255,255,.28)";
  const card={background:"rgba(255,255,255,.033)",border:"1px solid rgba(255,255,255,.07)",borderRadius:"10px",padding:"14px",marginBottom:"10px"};
  const gcard={background:"rgba(212,175,55,.048)",border:"1px solid rgba(212,175,55,.2)",borderRadius:"10px",padding:"14px",marginBottom:"10px"};
  const rcard={background:"rgba(239,68,68,.06)",border:"1px solid rgba(239,68,68,.2)",borderRadius:"10px",padding:"16px"};
  const inp={background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.11)",borderRadius:"6px",color:TX,padding:"7px 10px",fontSize:"13px",outline:"none",width:"100%"};
  const btn=(v="")=>({padding:v==="sm"?"5px 10px":v==="xs"?"3px 8px":"9px 18px",borderRadius:"7px",cursor:"pointer",fontFamily:"'Outfit',sans-serif",fontWeight:600,fontSize:v==="sm"||v==="xs"?"11px":"13px",border:v==="danger"||v==="reset"?"1px solid rgba(239,68,68,.35)":"1px solid rgba(255,255,255,.09)",background:v==="gold"?G:v==="danger"?"rgba(239,68,68,.14)":v==="reset"?"rgba(239,68,68,.2)":v==="green"?"rgba(74,222,128,.15)":"rgba(255,255,255,.07)",color:v==="gold"?"#0b0b12":v==="danger"||v==="reset"?RE:v==="green"?GR:TX});
  const badge=c=>({display:"inline-flex",alignItems:"center",padding:"2px 7px",borderRadius:"4px",fontSize:"9px",fontWeight:700,letterSpacing:".1em",background:c==="g"?"rgba(74,222,128,.11)":c==="b"?"rgba(96,165,250,.11)":"rgba(212,175,55,.11)",color:c==="g"?GR:c==="b"?"#60a5fa":G,border:`1px solid ${c==="g"?"rgba(74,222,128,.22)":c==="b"?"rgba(96,165,250,.22)":"rgba(212,175,55,.22)"}`});
  const mc=done=>({background:done?"rgba(74,222,128,.04)":"rgba(255,255,255,.018)",border:`1px solid ${done?"rgba(74,222,128,.15)":"rgba(255,255,255,.05)"}`,borderRadius:"8px",padding:"10px",marginBottom:"8px"});
  const rh={fontFamily:"'Bebas Neue',sans-serif",fontSize:"17px",letterSpacing:".08em",color:G,marginBottom:"10px"};
  const sf={fontFamily:"'Bebas Neue',sans-serif",fontSize:"22px",color:G};
  const vs={fontSize:"9px",color:"rgba(255,255,255,.25)",fontWeight:700,letterSpacing:".15em"};
  ...
  const MatchRow=({m,court})=>(
    <div style={mc(m.completed)}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        {m.completed&&<div style={{textAlign:"right"}}><div style={sf}>{m.s1} – {m.s2}</div>{m.tiebreak&&<div style={{fontSize:"8px",color:"rgba(212,175,55,.6)"}}>TIEBREAK</div>}</div>}
      </div>
    </div>
  );
  ...
  // ── Loading ────────────────────────────────────────────────
  if(loading)return(
    <div style={{minHeight:"100vh",background:"#0b0b12",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Outfit',sans-serif"}}>
      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"36px",color:G,letterSpacing:".06em"}}>THE ANGELS</div>
      <div style={{fontSize:"10px",color:"rgba(212,175,55,.45)",letterSpacing:".2em",marginTop:"4px"}}>LOADING...</div>
      <div style={{marginTop:"20px",width:"22px",height:"22px",border:`2px solid rgba(212,175,55,.2)`,borderTopColor:G,borderRadius:"50%",animation:"spin .8s linear infinite"}}/>
    </div>
  );
  ...
  const regView=()=>(
    <div className="fade">
      <div style={gcard}>
        <div style={{fontSize:"9px",color:"rgba(212,175,55,.5)",letterSpacing:".35em"}}>PHÂN CÔNG & TÀI TRỢ</div>
        <div style={{fontSize:"12px",color:"rgba(255,255,255,.38)",marginTop:"3px"}}>Ai phụ trách hạng mục nào? Bấm đăng ký bên dưới 👇</div>
      </div>

      <div style={{...card,background:"rgba(212,175,55,.03)",border:"1px solid rgba(212,175,55,.12)",marginBottom:"14px",fontSize:"12px",color:"rgba(255,255,255,.5)",lineHeight:1.7}}>
        ...
        return(
          <div key={item.id} style={{...card,marginBottom:"10px"}}>
            ...
            {itemRegs.length>0&&(
              <div style={{marginBottom:isOpen?"10px":"0"}}>
                {itemRegs.map((r,i)=>{
                  ...
                  return(
                    <div key={r.ts||i} style={{display:"flex",alignItems:"flex-start",gap:"8px",padding:"6px 8px",background:"rgba(255,255,255,.025)",borderRadius:"6px",marginBottom:"4px"}}>
                      <div style={{flex:1}}>
                        <div style={{display:"flex",alignItems:"center",gap:"6px",flexWrap:"wrap"}}>
                          <span style={{fontSize:"13px",fontWeight:500}}>{r.name}</span>
                          <span style={badge(tl.c)}>{tl.text}</span>
                        </div>
                        {r.note&&<div style={{fontSize:"11px",color:DM,marginTop:"2px"}}>📝 {r.note}</div>}
                      </div>
                      <button onClick={()=>deleteReg(item.id,r.ts)} style={{...btn("xs"),background:"transparent",border:"none",color:"rgba(255,255,255,.25)",fontSize:"12px",padding:"2px 4px"}}>✕</button>
                    </div>
                  );
                })}
              </div>
            ...
            {itemRegs.length===0&&!isOpen&&(
              <div style={{fontSize:"11px",color:"rgba(255,255,255,.2)",marginTop:"6px"}}>Chưa có ai đăng ký</div>
            )}

            {/* Registration form */}
            {isOpen&&(
              <div style={{background:"rgba(212,175,55,.05)",border:"1px solid rgba(212,175,55,.15)",borderRadius:"8px",padding:"12px"}}>
      ...
      <div style={card}>
        ...
        {ITEMS.map(item=>{
          ...
          return(
            <div key={item.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
              <span style={{fontSize:"12px",color:done?TX:DM}}>{item.icon} {item.title}</span>
              {done?<span style={badge("g")}>{itemRegs.length} người</span>:<span style={{fontSize:"10px",color:"rgba(255,255,255,.2)"}}>Trống</span>}
            </div>
          );
        })}
      </div>
  ...
  const scheduleView=()=>{
    ...
    return(
      <div className="fade">
        <div style={gcard}><div style={{fontSize:"9px",color:"rgba(212,175,55,.5)",letterSpacing:".35em"}}>LỊCH THI ĐẤU</div><div style={{fontSize:"12px",color:"rgba(255,255,255,.38)",marginTop:"3px"}}>Chủ nhật 7/6/2026 · 7:00 sáng · 2 sân</div></div>
        {matches[1].length===0&&<div style={{...card,textAlign:"center",padding:"36px",color:DM,fontSize:"13px"}}>⏳ BTC chưa nhập cặp đấu Lượt 1</div>}
        {ALL_TIMES.map(t=>{const slot=slotMap[t];if(!slot)return null;return<div key={t} style={card}><div style={{fontSize:"10px",color:"rgba(212,175,55,.75)",fontWeight:700,letterSpacing:".12em",marginBottom:"8px"}}>⏰ {t}</div>{[1,2].map(c=>slot[c]?<MatchRow key={c} m={slot[c]} court={c}/>:null)}</div>;})}
  ...
  // ── Standings ──────────────────────────────────────────────
  const standingsView=()=>{
    const rc=["#D4AF37","#C0C0C0","#CD7F32","rgba(74,222,128,.85)"];
    return(
      <div className="fade">
        <div style={gcard}><div style={{fontSize:"9px",color:"rgba(212,175,55,.5)",letterSpacing:".35em"}}>BẢNG XẾP HẠNG</div><div style={{fontSize:"12px",color:"rgba(255,255,255,.35)",marginTop:"3px"}}>Top 4 tiến vào bán kết</div></div>
        <div style={card}>
          <div style={{display:"grid",gridTemplateColumns:"28px 1fr 34px 34px 54px",gap:"6px",paddingBottom:"8px",borderBottom:"1px solid rgba(255,255,255,.05)",fontSize:"9px",color:DM,letterSpacing:".12em",fontWeight:700}}><span>#</span><span>VĐV</span><span style={{textAlign:"center"}}>T</span><span style={{textAlign:"center"}}>B</span><span style={{textAlign:"right"}}>ĐM</span></div>
        ...
        {isRoundDone(3)&&<div style={{...card,background:"rgba(74,222,128,.04)",border:"1px solid rgba(74,222,128,.15)",textAlign:"center",padding:"12px"}}><div style={{fontSize:"12px",color:GR}}>✅ Vòng bảng hoàn tất</div><div style={{fontSize:"10px",color:"rgba(255,255,255,.35)",marginTop:"3px"}}>Top 4: {standings.slice(0,4).map(p=>p.name).join(" · ")}</div></div>}
      </div>
    );
  };
  ...
  const finalsView=()=>{
    ...
    return(
      <div className="fade">
        <div style={gcard}><div style={{fontSize:"9px",color:"rgba(212,175,55,.5)",letterSpacing:".35em"}}>VÒNG CHUNG KẾT</div></div>
        ...
        <div style={card}>
          ...
          {semis.length===0?<div style={{fontSize:"12px",color:"rgba(255,255,255,.25)",marginBottom:"10px"}}>Chưa tạo</div>:semis.map((sf2,i)=>(<div key={i} style={{...mc(sf2.completed),marginBottom:"6px"}}><div style={{fontSize:"9px",color:"rgba(212,175,55,.6)",letterSpacing:".1em",marginBottom:"5px"}}>{sf2.label}</div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:"13px",color:sf2.winner===sf2.p1?G:TX}}>{sf2.winner===sf2.p1?"→ ":""}{getName(sf2.p1)}</div><div style={{...vs,margin:"2px 0"}}>VS</div><div style={{fontSize:"13px",color:sf2.winner===sf2.p2?G:TX}}>{sf2.winner===sf2.p2?"→ ":""}{getName(sf2.p2)}</div></div>{sf2.completed&&<div style={sf}>{sf2.s1}–{sf2.s2}</div>}</div></div>))}
          <div style={{...lbl,margin:"10px 0 8px"}}>CHUNG KẾT</div>
          {final?<div style={{...mc(final.completed),background:final.completed?"rgba(212,175,55,.06)":undefined}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:"14px",fontWeight:600,color:final.winner===final.p1?G:TX}}>{final.winner===final.p1?"👑 ":""}{getName(final.p1)}</div><div style={{...vs,margin:"3px 0"}}>VS</div><div style={{fontSize:"14px",fontWeight:600,color:final.winner===final.p2?G:TX}}>{final.winner===final.p2?"👑 ":""}{getName(final.p2)}</div></div>{final.completed&&<div style={{...sf,fontSize:"28px"}}>{final.s1}–{final.s2}</div>}</div>{final.completed&&<div style={{marginTop:"10px",textAlign:"center",padding:"8px",background:"rgba(212,175,55,.08)",borderRadius:"6px"}}><div style={{fontSize:"8px",color:"rgba(212,175,55,.6)",letterSpacing:".22em"}}>VÔ ĐỊCH</div><div style={{fontSize:"17px",fontWeight:700,color:G,marginTop:"2px"}}>👑 {getName(final.winner)}</div></div>}</div>:<div style={{fontSize:"12px",color:"rgba(255,255,255,.25)"}}>Chờ kết quả bán kết</div>}
          {semis.filter(sf2=>sf2.completed).length===2&&<><div style={{...lbl,margin:"10px 0 8px"}}>🥉 ĐỒNG HẠNG BA</div><div style={mc(true)}>{semis.map((sf2,i)=>{const loser=sf2.winner===sf2.p1?sf2.p2:sf2.p1;return<div key={i} style={{fontSize:"13px",padding:"2px 0"}}>🥉 {getName(loser)}</div>;})}<div style={{fontSize:"9px",color:"rgba(255,255,255,.25)",marginTop:"4px"}}>Không cần đấu thêm</div></div></>}
        </div>
      </div>
    );
  };
  ...
  const rulesView=()=>(
    <div className="fade">
      ...
      <div style={{...gcard,textAlign:"center",padding:"20px 16px"}}>
        ...
        <div style={{fontSize:"10px",color:"rgba(212,175,55,.5)",letterSpacing:".25em",marginTop:"2px"}}>TENNIS TOURNAMENT · 07.06.2026</div>
      </div>

      {/* I. Thông tin giải */}
      <div style={card}>
        <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"14px",paddingBottom:"10px",borderBottom:"1px solid rgba(212,175,55,.15)"}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"13px",color:G,background:"rgba(212,175,55,.15)",border:"1px solid rgba(212,175,55,.3)",borderRadius:"4px",padding:"2px 8px",letterSpacing:".1em"}}>I</div>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"16px",letterSpacing:".08em",color:G}}>THÔNG TIN GIẢI ĐẤU</div>
        </div>

        {/* Thời gian */}
        <div style={{marginBottom:"14px"}}>
          <div style={{fontSize:"10px",color:"rgba(212,175,55,.65)",letterSpacing:".15em",fontWeight:700,marginBottom:"8px"}}>🗓 THỜI GIAN</div>
          <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
            {[
              {label:"Tập trung",value:"06:30 · Chủ nhật 07/06/2026"},
              {label:"Thi đấu",value:"07:30 – 11:30"},
            ].map(row=>(
              <div key={row.label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",background:"rgba(255,255,255,.025)",borderRadius:"6px"}}>
        ...
        {/* Địa điểm */}
        <div style={{marginBottom:"14px"}}>
          <div style={{fontSize:"10px",color:"rgba(212,175,55,.65)",letterSpacing:".15em",fontWeight:700,marginBottom:"8px"}}>📍 ĐỊA ĐIỂM</div>
          <div style={{padding:"10px 12px",background:"rgba(255,255,255,.025)",borderRadius:"6px"}}>
        ...
        {/* Giải thưởng */}
        <div>
          <div style={{fontSize:"10px",color:"rgba(212,175,55,.65)",letterSpacing:".15em",fontWeight:700,marginBottom:"8px"}}>🏆 GIẢI THƯỞNG</div>
          <div style={{display:"flex",gap:"8px"}}>
            {[
              {rank:"🥇",label:"Vô địch",color:"#D4AF37",bg:"rgba(212,175,55,.1)"},
      ...
      <div style={card}>
        <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"14px",paddingBottom:"10px",borderBottom:"1px solid rgba(212,175,55,.15)"}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"13px",color:G,background:"rgba(212,175,55,.15)",border:"1px solid rgba(212,175,55,.3)",borderRadius:"4px",padding:"2px 8px",letterSpacing:".1em"}}>II</div>
        ...
        {[
          ...
          <div key={section.num} style={{marginBottom:"14px",paddingBottom:"14px",borderBottom:"1px solid rgba(255,255,255,.05)"}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"8px"}}>
              <div style={{width:"20px",height:"20px",borderRadius:"50%",background:"rgba(212,175,55,.15)",border:"1px solid rgba(212,175,55,.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"11px",fontWeight:700,color:G,flexShrink:0}}>{section.num}</div>
            ...
            <div style={{paddingLeft:"28px",display:"flex",flexDirection:"column",gap:"5px"}}>
              {section.items.map((item,i)=>(
                <div key={i} style={{display:"flex",gap:"8px",alignItems:"flex-start"}}>
                  <div style={{width:"4px",height:"4px",borderRadius:"50%",background:"rgba(212,175,55,.5)",marginTop:"6px",flexShrink:0}}/>
      ...
      <div style={card}>
        <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"14px",paddingBottom:"10px",borderBottom:"1px solid rgba(212,175,55,.15)"}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"13px",color:G,background:"rgba(212,175,55,.15)",border:"1px solid rgba(212,175,55,.3)",borderRadius:"4px",padding:"2px 8px",letterSpacing:".1em"}}>III</div>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"16px",letterSpacing:".08em",color:G}}>LUẬT THI ĐẤU</div>
        </div>

        {/* Vòng loại */}
        <div style={{marginBottom:"14px",paddingBottom:"14px",borderBottom:"1px solid rgba(255,255,255,.05)"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
            {[
              ...
              <div key={i} style={{display:"flex",alignItems:"flex-start",gap:"10px",padding:"8px 10px",background:"rgba(255,255,255,.025)",borderRadius:"6px"}}>
        ...
        <div>
          <div style={{fontSize:"12px",fontWeight:700,color:TX,marginBottom:"10px",display:"flex",alignItems:"center",gap:"6px"}}>
            <span style={{background:"rgba(212,175,55,.12)",border:"1px solid rgba(212,175,55,.25)",borderRadius:"4px",padding:"1px 7px",fontSize:"10px",color:G,fontWeight:700,letterSpacing:".05em"}}>BÁN KẾT & CHUNG KẾT</span>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
            {[
      ...
      {/* Footer */}
      <div style={{textAlign:"center",padding:"16px",color:"rgba(255,255,255,.2)",fontSize:"11px"}}>
    ...
    return(
      <div className="fade">
        ...
        <div style={card}>
          ...
          {players.map(p=>(<div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,.04)"}}><span style={{fontSize:"13px"}}>{p.id}. {p.name}</span><button style={btn("danger")} onClick={()=>{if(allGroupMatches.some(m=>m.p1===p.id||m.p2===p.id))return;update(prev=>({...prev,players:prev.players.filter(x=>x.id!==p.id)}));}}>✕</button></div>))}
        ...
        {semis.length>0&&<div style={card}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px"}}><div style={rh}>🏅 BÁN KẾT</div>{semis.every(sf2=>sf2.completed)&&<span style={badge("g")}>✓</span>}</div>{semis.map((sf2,i)=>(<div key={i} style={mc(sf2.completed)}><div style={{fontSize:"10px",color:"rgba(212,175,55,.65)",marginBottom:"7px"}}>{sf2.label}: {getName(sf2.p1)} vs {getName(sf2.p2)}</div><div style={{display:"flex",gap:"8px",alignItems:"flex-end"}}><SI label={getName(sf2.p1)} val={sf2.s1} onChange={v=>updateSemi(i,"s1",v)}/><span style={{...vs,paddingBottom:"8px"}}>:</span><SI label={getName(sf2.p2)} val={sf2.s2} onChange={v=>updateSemi(i,"s2",v)}/></div>{sf2.completed&&<div style={{marginTop:"6px"}}><span style={badge("g")}>✓ {getName(sf2.winner)} thắng</span></div>}</div>))}{semis.every(sf2=>sf2.completed)&&!final&&<button style={{...btn("gold"),width:"100%",marginTop:"8px"}} onClick={createFinal}>🏆 Tạo Chung kết</button>}</div>}
        {final&&<div style={gcard}><div style={rh}>🏆 CHUNG KẾT</div><div style={{fontSize:"12px",color:"rgba(255,255,255,.4)",marginBottom:"10px"}}>{getName(final.p1)} vs {getName(final.p2)}</div><div style={{display:"flex",gap:"8px",alignItems:"flex-end"}}><SI label={getName(final.p1)} val={final.s1} onChange={v=>updateFinal("s1",v)}/><span style={{...vs,paddingBottom:"8px"}}>:</span><SI label={getName(final.p2)} val={final.s2} onChange={v=>updateFinal("s2",v)}/></div>{final.completed&&<div style={{marginTop:"12px",textAlign:"center",padding:"12px",background:"rgba(212,175,55,.08)",borderRadius:"8px"}}><div style={{fontSize:"8px",color:"rgba(212,175,55,.6)",letterSpacing:".22em"}}>👑 VÔ ĐỊCH GIẢI</div><div style={{fontSize:"19px",fontWeight:700,color:G,marginTop:"4px"}}>{getName(final.winner)}</div></div>}</div>}

        <div style={{marginTop:"28px",paddingTop:"16px",borderTop:"1px solid rgba(239,68,68,.12)"}}>
          <div style={rcard}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"16px",letterSpacing:".08em",color:RE,marginBottom:"8px"}}>⚠️ VÙNG NGUY HIỂM</div><div style={{fontSize:"12px",color:"rgba(255,255,255,.38)",marginBottom:"14px",lineHeight:1.6}}>Xoá toàn bộ trận đấu, kết quả, cặp đấu.<br/>Danh sách VĐV được giữ lại. Không thể hoàn tác.</div><button style={{...btn("reset"),width:"100%"}} onClick={()=>setShowResetConfirm(true)}>🔄 Reset toàn bộ giải đấu</button></div>
        </div>
      </div>
    );
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0b0b12",fontFamily:"'Outfit',sans-serif",color:TX}}>
      ...
      <div style={{padding:"20px 16px 12px",textAlign:"center",background:"linear-gradient(180deg,rgba(212,175,55,.12) 0%,transparent 100%)",borderBottom:"1px solid rgba(212,175,55,.18)"}}>
        <div style={{fontSize:"9px",color:"rgba(212,175,55,.5)",letterSpacing:".35em",marginBottom:"5px"}}>🎾 GIẢI TENNIS NỘI BỘ</div>
        <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(30px,9vw,56px)",letterSpacing:".05em",color:G,textShadow:"0 0 50px rgba(212,175,55,.28)",lineHeight:1}}>THE ANGELS</h1>
        <div style={{fontSize:"10px",color:"rgba(212,175,55,.4)",letterSpacing:".22em",marginTop:"4px"}}>TENNIS TOURNAMENT · 07.06.2026</div>
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",padding:"4px 16px",background:"rgba(0,0,0,.35)",borderBottom:"1px solid rgba(255,255,255,.04)",fontSize:"10px",color:"rgba(255,255,255,.25)"}}>
      ...
      <div style={{display:"flex",background:"rgba(0,0,0,.4)",borderBottom:"1px solid rgba(255,255,255,.05)",overflowX:"auto"}}>
        {TABS.map(t=>(<button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"11px 14px",fontSize:"12px",fontWeight:500,cursor:"pointer",whiteSpace:"nowrap",background:"none",border:"none",color:tab===t.id?G:"rgba(255,255,255,.36)",borderBottom:tab===t.id?"2px solid #D4AF37":"2px solid transparent",transition:"color .2s"}}>{t.l}</button>))}
      </div>
